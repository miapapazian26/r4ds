...
#some tools for creating/eliminating missing explicit values, ex. cells where you see an NA

#18.2.1
#last observation carried forward 
#a common use for missing values is as a data entry convenience
#when data is entered by hand, missing values indicate that the value in the previous row has been repeated
#ex. 
treatment <- tribble(
...
#you can fill the missing values with tidyr::fill(). it works like select, takes a set of columns 
treatment |>
  fill(everything())
#this treatment is called last observation carried forward (locf)
...
#sometimes you can hit an opposite problem where some concrete value actually represents a missing value. this arises in data generated by older software that doesn't have a proper way of representing missing values. it must use 99 or -999
#if possible, handle this when reading in the data: use the na argument to readr::read_csv() ex., read_csv(path, na = "99")
#if you discover the problem later, you can use dplyr::na_if():
x <- c(1, 4, 5, 7, -99)
na_if(x, -99)

#18.2.3
#NaN: one type of special missing value (not a number)
#it behaves like NA: 
x <- c(NA, NaN)
x * 10
#> [1]  NA NaN
...
is.na(x)
#> [1] TRUE TRUE
#in the rare case you need to distinguish an NA from an NaN, you can use is.nan(x)
#you will generally encounter a NaN when performing a mathematical operation that has an indeterminate result
0 / 0 
#> [1] NaN
0 * Inf
#> [1] NaN
...
#missing values can be implicitly missing if an entire row of data is simply absent from the data.
#ex. 
stocks <- tibble(
...
#the dataset has two missing observations 
#the price in the fourth quarter of 2020 is explicitly missing, because its value is NA
#the price for the first quarter of 2021 is implicitly missing because it simply does not appear in the data set

#one way to think about the difference is with this koan: 
...
#in other cases, explicit missings are forced upon you by the structure of the data and you need to get rid of them

#18.3.1 pivoting
#you have already seen one tool that can make implicit missings explicit - pivoting
...
#ex. if we pivot stocks to put the quarter in the columns, both missing values become explicit
stocks |>
  pivot_wider(
    names_from = qtr, 
    values_from = price
  )
#by default, making data longer preserves explicit missing values but if they are structurally missing values that only exist because the data is not tidy, you can drop them (making them implicit) by setting values_drop_na = TRUE
...
#ex, we know that all combinations of year and qtr should exist in the stocks data: 
stocks |>
  complete(year, qtr)
#typically you will call complete() with names of existing variables filling in the missing combinations
#sometimes the individual variables are themselves incomplete, so you can instead provide your own data 
#ex, you may know that the stocks dataset is supposed to run from 2019 to 2021 so you could supply those values for year: 
stocks |>
  complete(year = 2019:2021, qtr)
#if the range of a variable is correct, you can use full_seq(x, 1) to generate all values from min(x) to max(x) spaced out by 1
#in some cases, the complete set of observations can't be generated by a simple combination of variables
#you can do manually what complete does for you: create a data frame that contains all the rows that should exist, then combine it with the original dataset with full_join

#18.3.3 joins 
#this brings us to another important way of revealing implicitly missing observations: joins
#dplyr::anti_join(x, y) is a particularly useful tool here: it selects only the rows in x that don't have a match in y
#ex., we can use two anti_join()s to reveal that we are missing information for four airports and 722 planes mentioned in flights:
library(nycflights13)
